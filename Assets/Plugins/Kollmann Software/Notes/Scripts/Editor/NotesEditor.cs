...
namespace KollmannSoftware.Notes {
    ...
    public class NotesEditor : EditorWindow {
        ...
        /// <summary>
        /// If the selected object is an asset
        /// </summary>
        private bool selectedObjectIsAsset;

        /// <summary>
        /// Initialization
        /// </summary>
        [MenuItem("Window/Kollmann Software/Editor Tools/Notes %&n")]
        static void Init() {
        ...
        void OnEnable() {
            ...
            UnityEditor.SceneManagement.EditorSceneManager.sceneLoaded += EditorSceneManager_sceneLoaded;
            SelectionChanged();
        }

        private void EditorSceneManager_sceneLoaded(UnityEngine.SceneManagement.Scene arg0, UnityEngine.SceneManagement.LoadSceneMode arg1) {
            SceneReferences.Refresh();
        }
                        ...
                        using (new EditorGUILayout.HorizontalScope()) {
                            if (GUILayout.Button(LABEL_REPLY_BUTTON, GUILayout.Width(80))) {
                                ...
                                if (nComment.SaveSelectedObject) {
                                    if (selectedGUID == null || selectedGUID.Equals("")) {
                                        selectedGUID = CreateGUID();
                                    }
        ...
        /// <summary>
        /// Creates a GUID from the selected GameObject
        /// </summary>
        /// <returns>GUID for the selected GameObject</returns>
        string CreateGUID() {
            if (Selection.activeObject && Selection.activeGameObject != null) {
                var guid = System.Guid.NewGuid();
                SceneReferences.Instance.AddReference(Selection.activeGameObject, guid.ToString());
        ...
        string GetGUID(Transform t) {
            return SceneReferences.Instance.GetGUIDByGameObject(t.gameObject);
        }
        ...
        GameObject GetGameObjectByGUID(string guid) {
            return SceneReferences.Instance.GetGameObjectByGUID(guid);
        }
        ...
        void DrawAllNotes() {
            //Filter the search
            var notes = library.Notes.Where(q => (library.searchString.Length > 0 ? q.Description.ToLower().Contains(library.searchString.ToLower()) : true)).ToList();

            //Filter the "visibility" dropdown
            if (Selection.activeObject != null && (VisibilitySelection)library.VisbilitySelection != VisibilitySelection.All) {
                notes = notes.Where(q => (q.selectedObjectIsAsset || SceneReferences.Instance.HasReference(q.selectedObjectGUID)) && (selectedParentGUIDs.Any(g => g == q.selectedObjectGUID) || selectedGUID == q.selectedObjectGUID)).ToList();
            }

            //Filter the "show" dropdown
            if (((ShowSelection)library.ShowSelection) != ShowSelection.All) {
                notes = notes.Where(q => ((ShowSelection)library.ShowSelection) == ShowSelection.Resolved ? q.IsResolved : ((ShowSelection)library.ShowSelection) == ShowSelection.Open ? !q.IsResolved : false).ToList();
            }

            //Filter the "category" dropdown
            if (library.SelectedCategory != null) {
                notes = notes.Where(q => q.Category == library.SelectedCategory).ToList();
            }
            ...
            //    && (q.selectedObjectIsAsset || SceneReferences.Instance.HasReference(q.selectedObjectGUID) || (q.selectedObjectGUID == "" && (Selection.activeObject == null || (VisibilitySelection)library.VisbilitySelection == VisibilitySelection.All)))
            ...
            //    && (Selection.activeObject == null || (VisibilitySelection)library.VisbilitySelection == VisibilitySelection.All ? true : (selectedParentGUIDs.Any(g => g == q.selectedObjectGUID) || q.selectedObjectGUID == selectedGUID))).ToList();
                    ...
                    using (new EditorGUILayout.HorizontalScope()) {
                        if (GUILayout.Button(LABEL_SAVE_BUTTON, GUILayout.Width(80))) {
                            ...
                            if (nNote.SaveSelectedObject) {
                                ...
                                nNote.selectedObjectIsAsset = selectedObjectIsAsset;
                            }
